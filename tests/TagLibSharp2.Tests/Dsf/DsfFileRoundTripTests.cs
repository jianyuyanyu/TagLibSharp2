...
/// <summary>
/// Tests that verify DSF (DSD Stream File) data preservation through read-write-read cycles.
/// </summary>
[TestClass]
public class DsfFileRoundTripTests
{
	[TestMethod]
	public void MinimalFile_PreservesStructure ()
	{
		// Arrange
		var original = TestBuilders.Dsf.CreateWithMetadata (title: "Test Title", artist: "Test Artist");

		// Act
		var result1 = DsfFile.Read (original);
		...
		var result2 = DsfFile.Read (written.Span);
	...
	{
		// Arrange
		var original = TestBuilders.Dsf.CreateWithMetadata (title: "Original");

		var result = DsfFile.Read (original);
		...
		Assert.AreEqual ("Jazz", reloaded.Genre);
	...
	public void CoverArt_PreservesBinaryData ()
	{
		...
		var original = TestBuilders.Dsf.CreateWithMetadata (title: "With Art");
		var result = DsfFile.Read (original);
		Assert.IsTrue (result.IsSuccess, $"Parse failed: {result.Error}");
		var file = result.File!;
	...
	{
		// Arrange
		var original = TestBuilders.Dsf.CreateWithMetadata (title: "With MB IDs");
		var result = DsfFile.Read (original);
	...
	{
		// Arrange
		var original = TestBuilders.Dsf.CreateWithMetadata (title: "With ReplayGain");
		var result = DsfFile.Read (original);
		Assert.IsTrue (result.IsSuccess);
		var file = result.File!;
		var tag = file.EnsureId3v2Tag ();
	...
	{
		// Arrange
		var original = TestBuilders.Dsf.CreateWithMetadata (title: "Title", artist: "Artist");
		...
		var result2 = DsfFile.Read (written1.Span);
		Assert.IsTrue (result2.IsSuccess);
		var file2 = result2.File!;
		var written2 = file2.Render ();
		var size2 = written2.Length;

		var result3 = DsfFile.Read (written2.Span);
