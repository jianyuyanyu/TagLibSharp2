...
/// <summary>
/// Tests that verify Musepack (.mpc) data preservation through read-write-read cycles.
/// </summary>
[TestClass]
public class MusepackFileRoundTripTests
{
	[TestMethod]
	public void MinimalFile_PreservesStructure ()
	{
		// Arrange
		var original = TestBuilders.Musepack.CreateWithMetadata (title: "Test Title", artist: "Test Artist");

		// Act
		var result1 = MusepackFile.Read (original);
		...
		var result2 = MusepackFile.Read (written);
	...
	{
		// Arrange
		var original = TestBuilders.Musepack.CreateWithMetadata (title: "Original");

		var result = MusepackFile.Read (original);
	...
	public void CoverArt_PreservesBinaryData ()
	{
		...
		var original = TestBuilders.Musepack.CreateWithMetadata (title: "With Art");
		var result = MusepackFile.Read (original);
		Assert.IsTrue (result.IsSuccess, $"Parse failed: {result.Error}");
		var file = result.File!;
	...
	{
		// Arrange
		var original = TestBuilders.Musepack.CreateWithMetadata (title: "With MB IDs");
		var result = MusepackFile.Read (original);
	...
	{
		// Arrange
		var original = TestBuilders.Musepack.CreateWithMetadata (title: "With ReplayGain");
		var result = MusepackFile.Read (original);
		Assert.IsTrue (result.IsSuccess);
		var file = result.File!;
		var tag = file.EnsureApeTag ();
	...
	{
		// Arrange
		var original = TestBuilders.Musepack.CreateWithMetadata (title: "Title", artist: "Artist");
		...
		var result2 = MusepackFile.Read (written1);
		Assert.IsTrue (result2.IsSuccess);
		var file2 = result2.File!;
		var written2 = file2.Render (written1);
		var size2 = written2.Length;

		var result3 = MusepackFile.Read (written2);
