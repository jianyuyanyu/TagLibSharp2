...
/// <summary>
/// Tests that verify WavPack data preservation through read-write-read cycles.
/// </summary>
[TestClass]
public class WavPackFileRoundTripTests
{
	[TestMethod]
	public void MinimalFile_PreservesStructure ()
	{
		// Arrange
		var original = TestBuilders.WavPack.CreateWithMetadata (title: "Test Title", artist: "Test Artist");

		// Act
		var result1 = WavPackFile.Read (original);
		...
		var result2 = WavPackFile.Read (written);
	...
	{
		// Arrange
		var original = TestBuilders.WavPack.CreateWithMetadata (title: "Original");

		var result = WavPackFile.Read (original);
	...
	public void CoverArt_PreservesBinaryData ()
	{
		...
		var original = TestBuilders.WavPack.CreateWithMetadata (title: "With Art");
		var result = WavPackFile.Read (original);
		Assert.IsTrue (result.IsSuccess, $"Parse failed: {result.Error}");
		var file = result.File!;
	...
	{
		// Arrange
		var original = TestBuilders.WavPack.CreateWithMetadata (title: "With MB IDs");
		var result = WavPackFile.Read (original);
	...
	{
		// Arrange
		var original = TestBuilders.WavPack.CreateWithMetadata (title: "With ReplayGain");
		var result = WavPackFile.Read (original);
		Assert.IsTrue (result.IsSuccess);
		var file = result.File!;
		var tag = file.EnsureApeTag ();
	...
	{
		// Arrange
		var original = TestBuilders.WavPack.CreateWithMetadata (title: "Title", artist: "Artist");
		...
		var result2 = WavPackFile.Read (written1);
		Assert.IsTrue (result2.IsSuccess);
		var file2 = result2.File!;
		var written2 = file2.Render (written1);
		var size2 = written2.Length;

		var result3 = WavPackFile.Read (written2);
