...
/// <summary>
/// Tests that verify Ogg Opus data preservation through read-write-read cycles.
/// </summary>
[TestClass]
public class OggOpusFileRoundTripTests
{
	[TestMethod]
	public void MinimalFile_PreservesStructure ()
	{
		// Arrange
		var original = TestBuilders.Opus.CreateMinimalFile (title: "Test Title", artist: "Test Artist");

		// Act
		var result1 = OggOpusFile.Read (original);
		...
		var result2 = OggOpusFile.Read (written.Span);
	...
	{
		// Arrange
		var original = TestBuilders.Opus.CreateMinimalFile (title: "Original");

		var result = OggOpusFile.Read (original);
		...
		Assert.AreEqual ("Electronic", reloaded.Genre);
	...
	public void CoverArt_PreservesBinaryData ()
	{
		...
		var original = TestBuilders.Opus.CreateMinimalFile (title: "With Art");
		var result = OggOpusFile.Read (original);
		Assert.IsTrue (result.IsSuccess, $"Parse failed: {result.Error}");
		var file = result.File!;
		...
		file.VorbisComment!.AddPicture (picture);
	...
	{
		// Arrange
		var original = TestBuilders.Opus.CreateMinimalFile (title: "With MB IDs");
		var result = OggOpusFile.Read (original);
	...
	{
		// Arrange
		var original = TestBuilders.Opus.CreateMinimalFile (title: "With ReplayGain");
		var result = OggOpusFile.Read (original);
		Assert.IsTrue (result.IsSuccess);
		var file = result.File!;
		var tag = file.VorbisComment!;
	...
	{
		// Arrange
		var original = TestBuilders.Opus.CreateMinimalFile (title: "Title", artist: "Artist");
		...
		var result2 = OggOpusFile.Read (written1.Span);
		Assert.IsTrue (result2.IsSuccess);
		var file2 = result2.File!;
		var written2 = file2.Render (written1.Span);
		var size2 = written2.Length;

		var result3 = OggOpusFile.Read (written2.Span);
